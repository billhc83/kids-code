(function () {
  ...
  var BLOCKS = {
    ...
    serialavailable: {
      allowed: [], asStatement: false, asExpr: true, // Added this block type
      inputs: [],
      ...
      if (inp.t === 'expr') {
        (function (capturedJi, capturedExNode) {
          ...
          subSlot.addEventListener('click', function (e) {
            ...
            } // Clear selection if already active
  ...
  function setupSection(elId, sName, label) {
    ...
    hdr.addEventListener('click', function (e) {
      ...
      if (PROGRESSION_MODE && guidance !== 'open') return;
  ...
  function renderForBlock(block, idx, parentArr, section, parentPathStr, anc) {
    ...
    var step = PROGRESSION_MODE && STEPS ? STEPS[CURRENT_STEP] : null;
    var guidance = step && step.config ? step.config.guidance : 'open';
    var canSelect = !PROGRESSION_MODE || guidance === 'open';
      ...
      if (tb.type === 'slot') { // Template is a slot
        if (ub === undefined || ub.type !== 'slot' || ub.content === null) { // User block is missing or slot is empty
          ...
          if (pm.expects === 'ifblock') {
            collectBadIds(uc.ifbody, pm.ifbody, tier, badIds);
  ...
  function validateStep() {
    ...
    document.querySelectorAll('.block-hint').forEach(function (el) { el.remove(); });
    if (!valid) {
      CHECK_FAIL_COUNT++; badIds.forEach(function (bid) {
  ...
  function buildWorkspace(stepIdx, saves) {
    ...
    window.CURRENT_STEP_META = { guidance: curGuidance, view: step.config.interface };
    ...
    window.dispatchEvent(new CustomEvent('bb_next_state', { detail: { state: { ready: false, 'check-mode': false, 'next-mode': false, hidden: !nextBtnState.visible, text: nextBtnState.text, prevVisible: nextBtnState.prevVisible, feedback: null } } }));
  ...
  window.bbNext = function () {
    ...
    if (nextBtnState.mode === 'check-mode') {
      if (validateStep()) {
        nextBtnState.text = 'Next Step \u2192'; nextBtnState.mode = 'next-mode';
    ...
    try {
      STUDENT_SAVES[CURRENT_STEP] = JSON.parse(JSON.stringify(SECTIONS));
